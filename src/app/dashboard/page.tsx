  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const paymentColors: Record<string, string> = { cash: "var(--success)", momo: "var(--chart-2)", card: "var(--warning)" }
        const paymentBreakdown = Object.entries(paymentTotals).map(([method, amount]) => ({
          name: method.charAt(0).toUpperCase() + method.slice(1),
          value: Math.round((amount / totalPayment) * 100),
          color: paymentColors[method] || "var(--muted-foreground)"
        }))
          ...
          <Card className="border-border/60">
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                ...
                <Link href="/reports">
                  <Button variant="ghost" size="sm" className="text-muted-foreground text-xs gap-1 rounded-xl">
                ...
                <div className="space-y-3">
                  ...
                  {data.lowStockItems.length > 4 && (
                    <Link href="/inventory" className="block pt-1">
                      <Button variant="outline" size="sm" className="w-full text-xs gap-2 rounded-xl">
