...
export default function CustomersPage() {
  ...
  const handleUpdate = async () => {
    ...
    setSaving(true)
    try {
      await fetch(`/api/customers?id=${selectedCustomer._id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(formData),
      })
  ...
  const handleDelete = async () => {
    if (!selectedCustomer) return
    try {
      await fetch(`/api/customers?id=${selectedCustomer._id}`, { method: "DELETE" })
        ...
        {filteredCustomers.length === 0 ? (
          <Card className="border-border/60">
            <CardContent className="p-0">
              <EmptyState
                ...
                description="Add customers to manage relationships and track purchases"
                action={
                  <Button size="sm" className="gap-2 rounded-xl" onClick={() => setShowAddModal(true)}>
                    <Plus className="h-4 w-4" />
                    Add Customer
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {filteredCustomers.map((customer) => (
              <Card key={customer._id} className="border-border/60 hover:shadow-card-hover transition-all duration-300">
                <CardContent className="p-6">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center justify-between pt-2 border-t border-border/60">
                      ...
                      <span className="text-sm font-bold">{formatCurrency(customer.totalSpent)}</span>
                    </div>
                  </div>
                </CardContent>
              </Card>
        ...
        <Dialog open={showAddModal} onOpenChange={setShowAddModal}>
          <DialogContent className="sm:max-w-md">
            ...
            <DialogFooter>
              ...
              <Button size="sm" className="text-sm" onClick={handleSave} disabled={saving || !formData.name || !formData.phone}>
          ...
          <DialogContent className="sm:max-w-md">
            ...
            <div className="py-4">
              <p className="text-sm text-muted-foreground">
                Are you sure you want to delete <span className="font-medium text-foreground">{selectedCustomer?.name}</span>? This action cannot be undone.
              </p>
            </div>
            <DialogFooter>
